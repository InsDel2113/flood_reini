...
partial class FloodPlayer : Player
{
	...
	{
		if ( FloodGame.Instance.CurrentRound != FloodGame.Round.PreGame )
		{
	...
	int hurtCounter = 0;
	public override void Touch( Entity other ) // ported from old flood
	{
		hurtCounter++;
		if ( other.ClassInfo.Name == "env_sea" && hurtCounter % 30 == 1 && FloodGame.Instance.CurrentRound == FloodGame.Round.Fight )
		{
			DamageInfo damageCause = new DamageInfo();
			damageCause.Damage = 1;
			TakeDamage( damageCause );
			if ( hurtCounter > 4 )
			{
				hurtCounter = 0;
			}
