...
namespace Sandbox
{
	public class WaterController
	{
		...
		{
			if ( other.WaterLevel.Entity != WaterEntity )
				return;
		...
		public virtual void OnEnterWater( ModelEntity other )
		{
			...
			other.WaterLevel.Entity = WaterEntity;
		}
		...
		private void UpdateBody( Entity ent, PhysicsBody body )
		{
			...
			//DebugOverlay.Box( WaterEntity.Position, (WaterEntity as AnimEntity).CollisionBounds.Mins, (WaterEntity as AnimEntity).CollisionBounds.Maxs, Color.Red, false );

			var oldLevel = body.WaterLevel;
			var density = body.Density;
			var waterSurface = WaterEntity.Position + (WaterEntity as AnimEntity).CollisionBounds.Maxs;
